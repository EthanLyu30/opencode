  ...
  return Effect.gen(function* () {
    ...
    if (!row.store) {
      ...
      yield* db
        ...
        .run()
        .pipe(Effect.orDie)
      const redactedCreated = yield* db
        .update(EventTable)
        .set({
          data: {
            responseID: row.id,
        ...
        .where(
          and(
            eq(EventTable.aggregate_id, data.responseID),
            eq(
              EventTable.type,
              EventV2.versionedType(ResponseEvent.Created.type, ResponseEvent.Created.durable!.version),
            ),
          ),
        )
        ...
        .get()
        .pipe(Effect.orDie)
      if (!redactedCreated) throw new LifecycleConflict(data.responseID)
    ...
    yield* events.project(ResponseEvent.Created, (event) =>
      Effect.gen(function* () {
        ...
        const workflow = yield* db
          ...
          .get()
          .pipe(Effect.orDie)
        if (!workflow) throw new LifecycleConflict(data.responseID)
