...
Fix round 2 addresses the remaining Important finding from the scoped Fix1 re-review, with 1 addressed and 0 open in this round. The implementation range is `f064efd16b147b6933f7d64711bbdc26aeb78f49..96186fecf0c9dd9923689b7f28127b31f0ebc815`. This is implementation and self-verification evidence only; it does not claim that an independent re-review is clean or complete.

The prior HTTP API codegen exception trusted the public `isPropertyNames` metadata shape. A custom always-failing predicate could therefore forge `_tag`, `propertyNames`, `~structural`, and `arbitrary` metadata, pass portability validation, and be silently regenerated as Effect's built-in `Schema.isPropertyNames`. The deterministic RED first proved the original schema rejected `{ SAFE_NAME: "value" }`, then failed because codegen accepted it and emitted the built-in predicate: **0 pass, 1 fail, 2 assertions**.

The forgeable exception is now removed. Property-name filters whose runtime source identity cannot be proven fail closed just like custom, unknown, spoofed, and aborted filters; adding more forgeable metadata does not establish authority. The complete metadata spoof and the real built-in-but-unproven filter are both rejected, and the full HTTP API codegen suite passes **64/64, 123 assertions**.

`WorkflowVisualBuild` no longer needs that exception. Its environment is a keyed `Schema.Record(EnvironmentName, EnvironmentValue)`, using already portable `Schema.isPattern` checks on both key and value. Protocol runtime coverage accepts safe string values and rejects numbers, malformed names, and newline values: **11/11, 22 assertions**. The official Effect client remains portable and `packages/client` passes **22/22, 117 assertions** plus `check:generated` with no diff.

Raw Effect OpenAPI represents the keyed record as one fully anchored `patternProperties` entry. The existing public OpenAPI transform now normalizes only an object with no `properties` or `additionalProperties`, exactly one pattern entry, and an unescaped leading `^` and trailing `$`. It preserves the value schema as `additionalProperties` and the key constraint as `propertyNames`; the partial `^ses` record is explicitly proven unchanged. The exact OpenAPI test passes **1/1, 7 assertions**, with `additionalProperties: string` and the environment-name pattern, while the SDK-next real HTTP matrix passes **5/5, 38 assertions**, including numeric environment rejection with `400`.

The authoritative v2 SDK type remains `{ [key: string]: string }`. Its contract accepts a string environment and consumes `@ts-expect-error` on a numeric value under the SDK test typecheck. The official SDK build was run in two clean deterministic rounds: each 15-file Git-canonical v2 generated manifest was **1,080 bytes**, SHA-256 `C8E32267143C963A3F8B4A7BCDF4A3FA5BEDAC98A4DA159066393849DC271297`; each generated diff was **0 bytes**, SHA-256 `E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855`. A post-commit official build reproduced the same manifest, removed its temporary `openapi.json`, and left the repository clean. SDK runtime coverage passes **3/3, 6 assertions**.

Fresh related gates also pass: Schema source-path/visual safety **9/9, 46 assertions**; OpenCode OpenAPI/auth/generated-client composition **47/47, 262 assertions**; and Core, HTTP API codegen, Schema, Protocol, Server, Client, SDK-next, SDK-js, and OpenCode typechecks all exit 0. The OpenCode matrix used the already established stable local-only `NO_PROXY=0.0.0.0,127.0.0.1,localhost`; no timeout or product behavior was changed.

Exact five-file Prettier and `git diff --check` exit 0. Oxlint reports 0 errors and 0 changed-line diagnostics; its two warnings are inherited at unchanged test line 809. The final diff/security audit finds no sensitive filename, secret signature, non-local URL, generated drift, diagnostic OpenAPI file, or residual Bun/Node process. No provider, external network, browser, Docker, paid, deployment, push, or secret-reading action was performed. The exact task-owned `D:\OpenCode-Task23.9-Fix2` root is confined to non-reparse task cache, experiment, and test-temp artifacts and is removed through the Windows Recycle Bin during final handoff; repository inputs and user-owned files are preserved.

### Controller adjudication of the Fix2 scoped re-review

The scoped reviewer confirmed that Fix2 removed the newly introduced forgeable `isPropertyNames` exception and that the product's keyed environment contract is aligned across runtime, Effect client, OpenAPI, and authoritative v2 SDK. It then demonstrated that the repository's older generic portable-filter contract also allows a custom source filter to forge the complete metadata of an existing built-in such as `isPattern`.

That generic behavior is unchanged from the Task23.9 base: `checksPortable` already treated repository-authored `meta` plus `arbitrary.constraint` as trusted reconstruction metadata. Code generation operates on trusted local repository source, not untrusted request data; a malicious source author could directly modify the generator or generated output. The controller therefore ruled the generic provenance observation out of Task23.9 scope rather than attempting a partial endpoint-specific fix. If the threat model later treats repository schema authors as untrusted, a separate cross-cutting task must replace metadata trust for every portable filter. The cost of this ruling is that a malicious or mistaken schema author can still forge built-in metadata and make generated validation differ from runtime; no such spoof exists on the visual-build path, whose four representations are verified equal.
